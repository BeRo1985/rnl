/******************************************************************************
 *                        RNL EXTENDED MASTER SECRET PROBE                    *
 ******************************************************************************
 *                                                                            *
 * Asks a real relay whether it honours RFC 7627, and if it does, completes a *
 * handshake whose master secret is derived that way.                         *
 *                                                                            *
 * The question matters twice over. It decides whether TRNLDTLS12Client may   *
 * require the extension - it may not, if the answer is no - and if the       *
 * answer is yes it is the only outside anchor there can be for               *
 * DeriveExtendedMasterSecret, because RFC 7627 publishes no vectors at all.  *
 *                                                                            *
 * Same shape as capturedtlsvectors.c, and deliberately a separate file: that *
 * one produced the vectors which are already checked in, and rerunning it    *
 * must keep producing the same kind.                                         *
 *                                                                            *
 * Build:                                                                     *
 *   cd src/tools && clang -O2 -o emsprobe emsprobe.c -lcrypto                *
 *                                                                            *
 * Run:                                                                       *
 *   ./emsprobe [host] [port]                                                 *
 *                                                                            *
...
/* Whether the ServerHello carries the extended_master_secret extension back. Walks the fixed
   part first - version, random, session id, cipher suite, compression - and then the extension
   list, because there is no other way to reach it. */
static int server_hello_echoes_ems(const unsigned char *aServerHello, size_t aSize) {
  ...
  position += 1 + aServerHello[position];
  position += 2 + 1;
  if ((position + 2) > aSize) {
    return 0;
  }
  total = ((size_t)aServerHello[position] << 8) | aServerHello[position + 1];
  position += 2;
  end = position + total;
  if (end > aSize) {
    end = aSize;
  ...
  while ((position + 4) <= end) {
    unsigned type = ((unsigned)aServerHello[position] << 8) | aServerHello[position + 1];
    size_t length = ((size_t)aServerHello[position + 2] << 8) | aServerHello[position + 3];
    if (type == 23) {
      return 1;
    }
    position += 4 + length;
...
int main(int argc, char **argv) {
  ...
  if (!server_hello_echoes_ems(messages[2].body.data, messages[2].body.size)) {
    printf("# extended_master_secret echoed by %s:%s -> no\n", host, port);
    printf("# so the client must not require it, and there is no anchor to be had here\n");
    close(socket_handle);
    return 0;
  }
  printf("# extended_master_secret echoed by %s:%s -> yes\n", host, port);
